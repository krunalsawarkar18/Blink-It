...
function useSubscribe(props) {
	...
	import_react.useEffect(() => {
		const subscription = !props.disabled && _props.current.subject && _props.current.subject.subscribe({ next: _props.current.next });
		return () => {
...
function useFormState(props) {
	...
	const _mounted = import_react.useRef(true);
...
function useController(props) {
	...
	return {
		field: {
			...
			onChange: import_react.useCallback((event) => _registerProps.current.onChange({
				target: {
			...
			onBlur: import_react.useCallback(() => _registerProps.current.onBlur({
				target: {
			...
			ref: import_react.useCallback((elm) => {
				const field = get(control._fields, name);
...
function useFieldArray(props) {
	...
	const append = (value, options) => {
		...
		control._updateFieldArray(name, updatedFieldArrayValues, appendAt, { argA: fillEmptyArray(value) });
	};
	const prepend = (value, options) => {
		...
		updateValues(updatedFieldArrayValues);
		setFields(updatedFieldArrayValues);
		control._updateFieldArray(name, updatedFieldArrayValues, prependAt, { argA: fillEmptyArray(value) });
	};
	const remove = (index) => {
		...
		updateValues(updatedFieldArrayValues);
		setFields(updatedFieldArrayValues);
		control._updateFieldArray(name, updatedFieldArrayValues, removeArrayAt, { argA: index });
	};
	...
	import_react.useEffect(() => {
		...
		if (_actioned.current && (!getValidationModes(control._options.mode).isOnSubmit || control._formState.isSubmitted)) if (control._options.resolver) control._executeSchema([name]).then((result) => {
...
function createFormControl(props = {}) {
	...
	const onChange = async (event) => {
		...
		if (field) {
			...
			if (shouldSkipValidation) {
				if (_proxyFormState.isValid) if (props.mode === "onBlur") {
					if (isBlurEvent) _updateValid();
				} else _updateValid();
	...
	const watch = (name, defaultValue) => isFunction(name) ? _subjects.values.subscribe({ next: (payload) => name(_getWatch(void 0, defaultValue), payload) }) : _getWatch(name, defaultValue, true);
...
function useForm(props = {}) {
	const _formControl = import_react.useRef();
	const _values = import_react.useRef();
	...
	useSubscribe({
		subject: control._subjects.state,
		next: (value) => {
			if (shouldRenderFormState(value, control._proxyFormState, control._updateFormState, true)) updateFormState({ ...control._formState });
		}
	});
	...
	import_react.useEffect(() => {
		props.shouldUnregister && control._subjects.values.next({ values: control._getWatch() });
