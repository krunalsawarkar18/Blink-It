...
function find(iterable, comparator) {
	for (const entry of iterable) if (comparator(entry)) return entry;
}
	...
	return function createSlice2(options) {
		...
		function makeSelectorProps(reducerPath2, injected = false) {
			...
			function getSelectors(selectState = selectSelf) {
				return emplace(emplace(injectedSelectorCache, injected, { insert: () => /* @__PURE__ */ new WeakMap() }), selectState, { insert: () => {
					const map = {};
...
var createDynamicMiddleware = () => {
	...
	const getFinalMiddleware = (api) => {
		return compose(...Array.from(middlewareMap.values()).map((entry) => emplace(entry.applied, api, { insert: () => entry.middleware(api) })));
	};
...
var createStateProxy = (state, reducerMap) => emplace(stateProxyMap, state, { insert: () => new Proxy(state, { get: (target, prop, receiver) => {
